...
export default function TechTabs({ selected, onSelect }: TechTabsProps) {
  const handleKeyDown = useCallback(
    (e: React.KeyboardEvent, idx: number) => {
      ...
      if (e.key === "ArrowRight") {
        e.preventDefault();
        nextIdx = idx < techTabs.length - 1 ? idx + 1 : 0;
      } else if (e.key === "ArrowLeft") {
        e.preventDefault();
        nextIdx = idx > 0 ? idx - 1 : techTabs.length - 1;
      } else if (e.key === "Home") {
        e.preventDefault();
        nextIdx = 0;
      } else if (e.key === "End") {
        e.preventDefault();
        nextIdx = techTabs.length - 1;
      } else {
        return;
      }
      onSelect(techTabs[nextIdx].id);
  ...
  return (
    <div
      role="tablist"
      aria-label="Tech page sections"
      className="flex items-center h-9 bg-[rgba(236,236,240,0.5)] rounded-lg p-0.5 overflow-x-auto scrollbar-hide"
    >
      {techTabs.map((tab, idx) => {
        ...
        return (
          <button
            ...
            className={`flex items-center gap-1.5 shrink-0 px-2.5 h-8 rounded-md text-xs font-medium transition-colors focus:outline-none focus:ring-2 focus:ring-[#2b7fff]/20 ${
              isActive
